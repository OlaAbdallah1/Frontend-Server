...
router.use('/books/info/:id', async(req, res)=>{
    ...
    const url = 'http://192.168.56.102/books/info/'+id
...
router.use('/books/search/:topic', (req, res)=>{
    ...
    const url = 'http://192.168.56.102/books/search/'+topic
    request({url, json:true}, (error,  {body})=>{
           if(error){
               return res.staus(404).send(error)
           }
...
router.patch('/books/:id', (req, res)=>{
    const id = req.params.id
    const url = 'http://192.168.56.102/books/'+id
...
router.use('/books/purchase/:id', (req, res)=>{
    const id = req.params.id
    const url = 'http://192.168.56.101/books/purchase/'+id
    request({url, json:true}, (error, {body})=>{
        if(error){
